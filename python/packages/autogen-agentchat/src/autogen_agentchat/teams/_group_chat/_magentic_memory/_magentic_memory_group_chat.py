...
from ._magentic_memory_orchestrator import MagenticMemoryOrchestrator
...
class MagenticMemoryGroupChatConfig(BaseModel):
    """The declarative configuration for a MagenticMemoryGroupChat."""
...
class MagenticMemoryGroupChat(BaseGroupChat, Component[MagenticMemoryGroupChatConfig]):
    """A team that runs a group chat with participants managed by the MagenticMemoryOrchestrator.
    ...
    the MagenticMemoryGroupChat does not support using team as participant.
    ...
    MagenticMemoryGroupChat with one assistant agent:
        ...
        .. code-block:: python
            ...
            async def main() -> None:
                ...
                team = MagenticMemoryGroupChat([assistant], model_client=model_client)
    ...
    References:
        ...
        If you use the MagenticMemoryGroupChat in your work, please cite the following paper:
    ...
    component_config_schema = MagenticMemoryGroupChatConfig
    component_provider_override = "autogen_agentchat.teams.MagenticMemoryGroupChat"

    DEFAULT_NAME = "MagenticMemoryGroupChat"
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            group_chat_manager_name="MagenticMemoryOrchestrator",
            group_chat_manager_class=MagenticMemoryOrchestrator,
        ...
        if len(participants) == 0:
            raise ValueError("At least one participant is required for MagenticMemoryGroupChat.")
    ...
    ) -> Callable[[], MagenticMemoryOrchestrator]:
        return lambda: MagenticMemoryOrchestrator(
            name,
            group_topic_type,
            output_topic_type,
    ...
    def _to_config(self) -> MagenticMemoryGroupChatConfig:
        participants = [participant.dump_component() for participant in self._participants]
        termination_condition = self._termination_condition.dump_component() if self._termination_condition else None
        return MagenticMemoryGroupChatConfig(
            name=self.name,
    ...
    @classmethod
    def _from_config(cls, config: MagenticMemoryGroupChatConfig) -> Self:
