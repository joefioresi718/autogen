...
class MagenticMemoryOrchestrator(BaseGroupChatManager):
    ...
    async def _orchestrate_step(self, cancellation_token: CancellationToken) -> None:
        ...
        # Provide shared memory keys to the orchestrator every step
        try:
                ...
                with open(memory_file, "r") as f:
                    ...
                    if keys:
                        ...
                        context.append(
                            UserMessage(
                                content=(
                                    "Shared memory keys available from parallel teams. If any look relevant, propose fetching their values by key.\n\n"
                                    f"Keys (most recent last):\n{keys_text}"
                                ),
                                source="SharedMemory"
                            )
                        )
        except Exception:
            # Do not fail orchestration if shared memory is unavailable
            pass
